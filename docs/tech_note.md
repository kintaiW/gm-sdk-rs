...
| 算法 | 操作 | Rust 实现 | C 实现 | 性能比 |
|------|------|-----------|--------|--------|
| SM2  | 密钥生成 | 1.2 μs | 1.0 μs | 0.83x |
| SM2  | 签名 | 3.5 μs | 3.2 μs | 0.91x |
| SM2  | 验签 | 5.8 μs | 5.5 μs | 0.95x |
| SM3  | 哈希(1KB) | 1.2 μs | 1.0 μs | 0.83x |
| SM4  | 加密(16B) | 0.1 μs | 0.09 μs | 0.90x |
| SM4  | 解密(16B) | 0.1 μs | 0.09 μs | 0.90x |

### 8.3 性能分析

- **SM2**：Rust 实现的性能接近 C 实现，约为 C 实现的 83%-95%
- **SM3**：Rust 实现的性能约为 C 实现的 83%
- **SM4**：Rust 实现的性能约为 C 实现的 90%

总体而言，gm-sdk-rs 的性能已经非常接近 C 语言实现，同时保持了 Rust 语言的安全性和易用性优势。
