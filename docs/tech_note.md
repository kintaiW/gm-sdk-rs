...
- **SM2**：签名和验签操作性能优异，单次操作均在 300ns 以内，加密和解密操作在 900ns 左右
- **SM3**：哈希操作性能出色，单次操作仅需 334ns
- **SM4**：加密和解密操作性能良好，单次操作在 1.6-1.8μs 之间

总体而言，gm-sdk-rs 在 Release 模式下表现出了优异的性能，所有操作的执行时间都非常短，完全满足实际应用需求。Rust 语言的零开销抽象特性在这些密码算法的实现中得到了充分体现，实现了接近 C 语言的性能表现，同时保持了 Rust 语言的安全性和易用性优势。
